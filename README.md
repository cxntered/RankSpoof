<div align="center">

# `RankSpoof`

A mod to spoof your Hypixel Rank

This is the modern branch (`v2`), made for the latest versions of Minecraft. To see the legacy version (`v1`), made for 1.8.9, check out the [`legacy` branch](https://github.com/cxntered/RankSpoof/tree/legacy/ornithe).

Originally inspired by [`CustomRankUpdated`](https://www.chattriggers.com/modules/v/CustomRankUpdated) by Alexandra for ChatTriggers (which is an updated version of [`CustomRank`](https://www.chattriggers.com/modules/v/CustomRank) by Heknon).

</div>

---

<details>
<summary>Gallery</summary>

![Showcase](images/showcase.png)
![RGB Colors Showcase](images/rgb-colors.png)
![Configuration Menu](images/config-menu.png)
</details>
